...
class Team {
  ...
  updatePQ(newTeammate) {
    const totalUsage = sumObjValues(this.data[newTeammate].Abilities);
    for (let i = 0; i < this.pq.elements.length; i += 1) {
      const { data: pokemonName, priority: oldUseRate } = this.pq.elements[i];
      const useRateChange = this.correlations[newTeammate][pokemonName] / totalUsage;
      const newUseRate = oldUseRate + useRateChange;
      if (Number.isNaN(newUseRate)) {
        throw new Error(
          `${newTeammate} ${pokemonName} ${totalUsage} ${newUseRate} ${oldUseRate} ${useRateChange} ${this.correlations[newTeammate][pokemonName]}`,
        );
      }
